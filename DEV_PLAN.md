...
> 最后更新：2026-09-07（登录态失效保护、metadata 并发初始化与 24 小时会话）
> 当前分支：main
...
- 工作台/邮箱/AI 配置/接待状态收到 `401` 时统一停止旧数据渲染，显示刷新并重新登录入口。
- 原生页面捕获 GraphQL `UNAUTHENTICATED` 和注入接口 `401`，避免登录失效后继续显示空壳。
- Portal 为 `/metadata` 增加按认证身份隔离的 15 秒短缓存、并发锁和无临时磁盘缓冲，降低多用户首屏重复初始化压力。
- 服务端访问令牌默认有效期调整为 24 小时，刷新令牌保留 90 天；令牌过期优先自动续期，刷新令牌失效或被撤销时才要求重新登录。
- metadata 缓存启用后台刷新和短暂 stale 容错，避免 TTL 刚到期或上游瞬时异常造成全站首屏等待。
- 对话工作台改为游标分页：首批 30 条，触底追加 30 条；权限过滤、时间排序和实时刷新逻辑保持在分页边界内。
