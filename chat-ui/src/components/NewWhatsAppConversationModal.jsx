    ...
    <div style={overlayStyle} role="presentation">
      <div role="dialog" aria-modal="true" aria-labelledby="new-wa-title" style={{
        ...
        <div style={{ padding: 16, display: 'grid', gap: 14 }}>
          {!confirming ? <>
            <label style={{ display: 'grid', gap: 6, fontSize: 12, color: 'var(--text-secondary)' }}>
              WhatsApp 号码
              <input autoFocus value={phone} onChange={(event) => setPhone(event.target.value)} placeholder="包含国家区号，例如 +1 202 555 0147" style={{ height: 36, border: '1px solid var(--border)', borderRadius: 6, padding: '0 10px', background: 'var(--bg-surface)', color: 'var(--text-primary)', outline: 'none' }} />
            </label>
            <label style={{ display: 'grid', gap: 6, fontSize: 12, color: 'var(--text-secondary)' }}>
              首条消息
              <textarea value={content} maxLength={4096} onChange={(event) => setContent(event.target.value)} placeholder="输入发送给客户的第一条消息" rows={5} style={{ resize: 'vertical', minHeight: 100, border: '1px solid var(--border)', borderRadius: 6, padding: 10, background: 'var(--bg-surface)', color: 'var(--text-primary)', outline: 'none', fontFamily: 'inherit' }} />
            </label>
          </> : <div style={{ padding: '8px 0', display: 'grid', gap: 10 }}>
            <strong style={{ fontSize: 14 }}>确认主动联系该客户？</strong>
            <p style={{ fontSize: 12.5, color: 'var(--text-secondary)' }}>消息将通过当前绑定的 WhatsApp 账号发送至 <strong style={{ color: 'var(--text-primary)' }}>{phone}</strong>。发送后会话进入人工接管状态。</p>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8, padding: '12px 16px', borderTop: '1px solid var(--border)' }}>
          ...
          {!confirming ? <button onClick={() => { setError(''); if (!phone.trim() || !content.trim()) return setError('请填写 WhatsApp 号码和首条消息'); setConfirming(true) }} style={{ height: 34, padding: '0 14px', border: 0, borderRadius: 6, background: 'var(--green)', color: '#fff', cursor: 'pointer', fontWeight: 600 }}>下一步</button>
