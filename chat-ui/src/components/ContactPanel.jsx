...
export function ContactPanel({ conv }) {
  const [activeTab, setActiveTab] = useState('资料')

  if (!conv) return <div style={{ width: 300, borderLeft: '1px solid var(--border)', background: 'var(--bg-secondary)' }} />

  const c = conv.contact
  const channelLabel = conv.channel === 'whatsapp' ? 'WhatsApp' : conv.channel === 'website' ? '官网聊天' : conv.channel
  ...
  return (
    <div style={{
      ...
      {activeTab !== '资料' ? <PlaceholderTab label={activeTab} /> : (
