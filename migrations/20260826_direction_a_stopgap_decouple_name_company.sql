-- 方向A · 一期止血：解耦「线索主列 name」与「公司名」，公司统一走 company 关系
-- 删除 4 条 name<->公司名 同步；保留 项目.name <-> 客户.gongSiMingCheng；companyId 同步不动。
...
-- 线索主列 name 显示名统一为「名称」（底层 id 不变）
UPDATE core."fieldMetadata"
SET label = '名称', "standardOverrides" = jsonb_set(COALESCE("standardOverrides",'{}'::jsonb), '{label}', '"名称"')
