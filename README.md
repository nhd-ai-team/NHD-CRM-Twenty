...
| 对话消息实时性 | ✅ WhatsApp / 官网入站和 CRM 出站接入 SSE 通知；轮询保留为断线兜底 |
| 对话清单分页 | ✅ 首屏加载 30 条，滚动到底部按游标追加 30 条；实时刷新只更新已加载清单 |
| 交还会话稳定性 | ✅ 已修复确认弹窗 `permissions` 作用域错误导致的工作台白屏 |
| 登录态与主页面加载保护 | ✅ `401` / `UNAUTHENTICATED` 显示失效页；metadata 按认证身份短缓存并合并并发初始化 |
| 登录态有效期 | ✅ 访问令牌 24 小时；刷新令牌 90 天，支持自动续期 |

---

| 钉钉官网客服通知 | ✅ | 内部应用工作通知定向提醒销售；未分配时通知配置销售，人工接管后只通知当前负责人；失败进入持久化重试队列 |
...
| **新用户使用 CRM**：有哪些板块、能做什么、常用流程 | `docs/33-CRM使用手册（用户版）.md` |
| **在线查看用户手册** | [https://crm.chinanhd.com/crm-user-guide/](https://crm.chinanhd.com/crm-user-guide/) |
...
| 代码与文档对齐记录 | `docs/18`（08-18）、`docs/21`（08-19）、`docs/23`（08-24）、`docs/25`（08-31）、`docs/26`（09-01）、`docs/27`（09-01）、`docs/30`（09-09） |
